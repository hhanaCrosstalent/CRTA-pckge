...
(function(factory) {
  ...
  $.fn.orgchart = function(options) {
    ...
    if (opts.exportButton) {
		if($chartContainer.prev().hasClass('oc-export-btn') && $chartContainer.prev().hasClass($chartContainer.attr('id'))) {
		  $chartContainer.prev().remove();
		}
		var $exportBtn = $('<button/>', {
		  'class': 'oc-export-btn ' + $chartContainer.attr('id') + (opts.chartClass !== '' ? ' ' + opts.chartClass : ''),
		  'text': 'Export',
		  'click': function(e) {
			e.preventDefault();
			  ...
			  'onrendered': function(canvas) {
				...
				if (opts.exportFileextension.toLowerCase() === 'pdf') {
				  ...
				  if ((!isWebkit && !isFf) || isEdge) {
					...
					$chartContainer.find('.oc-download-btn').attr('href', canvas.toDataURL('image/png', 1.0))[0].click();
				  }
				}
			  }
			})
		...
		$exportBtn.insertBefore($chartContainer);
		if (opts.exportFileextension.toLowerCase() !== 'pdf') {
		  var oldDownloadBtn = jQuery('.oc-download-btn .' + $chartContainer.attr('id') + (opts.chartClass !== '' ? ' .' + opts.chartClass : ''));
		  if(oldDownloadBtn.length > 0) {
	        oldDownloadBtn.remove();
		  }
		  var downloadBtn = '<a class="oc-download-btn ' + $chartContainer.attr('id') + (opts.chartClass !== '' ? ' ' + opts.chartClass : '') + '"'
			+ ' download="' + opts.exportFilename + '.png"></a>';
		  $chartContainer.append(downloadBtn);
		}
    }
  ...
  function createNode(nodeData, level, opts) {
    ...
    var $nodeDiv = $('<div' + (opts.draggable ? ' draggable="true"' : '') + (nodeData[opts.nodeId] ? ' id="' + nodeData[opts.nodeId] + '"' : '') + (nodeData.parentId ? ' data-parent="' + nodeData.parentId + '"' : '') + ' depth="' + nodeData.level + '"' + '>')
    ...
    $nodeDiv.on('click', '.topEdge', function(event) {
      ...
      if (parentState.exist) {
        ...
        // load new nodes
		  var url;
	      if($.isFunction(opts.ajaxURL.parent)) {
			url = opts.ajaxURL.parent(nodeData);
		  } else {
			url = opts.ajaxURL.parent+ nodeId;
	      }
		  if(url.constructor === {}.constructor) {
			if (!$.isEmptyObject(url)) {
			  addParent.call($node.closest('.orgchart').parent(), $node, url, opts);
    ...
    $nodeDiv.on('click', '.bottomEdge', function(event) {
      ...
      if (childrenState.exist) {
        ...
        if (startLoading($that, $node, opts)) {
		  var url;
	      if($.isFunction(opts.ajaxURL.children)) {
			url = opts.ajaxURL.children(nodeData);
		  } else {
			url = opts.ajaxURL.children + nodeId;
	      }
		  if(url.constructor === {}.constructor) {
			if (url.children.length) {
			  addChildren($node, url, $.extend({}, opts, { depth: 0 }));
			}
			endLoading($that, $node, opts);
		  } else {
  ...
  function showChildren($node) {
    ...
    $nodeDiv.on('click', '.toggleBtn', function(event) {
      ...
      if ($descendants.eq(0).is('.slide-up')) {
		...
		var nodeId = $this.parent()[0].id;
        if (startLoading($this, $node, opts)) {
		  var url;
	      if($.isFunction(opts.ajaxURL.children)) {
			url = opts.ajaxURL.children(nodeData);
		  } else {
		  ...
		  if(url.constructor === {}.constructor) {
			url.depth = parseInt($node.attr('depth'));
			if (url.children.length) {
			  addChildren($node, url, $.extend({}, opts, { depth: 0 }));
			}
			endLoading($this, $node, opts);
		  } else {
			$.ajax({ 'url': $.isFunction(opts.ajaxURL.children) ? opts.ajaxURL.children(nodeData) : opts.ajaxURL.children + nodeId, 'dataType': 'json' })
			.done(function(data, textStatus, jqXHR) {
			  if ($node.closest('.orgchart').data('inAjax')) {
				data.depth = parseInt($node.attr('depth'));
			    if (data.children.length) {
			      addChildren($node, data, $.extend({}, opts, { depth: 0 }));
			    }
			  }
      ...
      var $that = $(this);
      var $node = $that.parent();
	  var $descWrapper = $that.parent().next();
	  var $descendants = $descWrapper.find('.node');
	  var $children = $descWrapper.children().children('.node');
	  var childrenState = getNodeState($node, 'children');
      ...
      if (childrenState.exist) {
        ...
        if ($children.is('.slide')) { return; }
        $that.toggleClass('fa-plus-square fa-minus-square');
    ...
    $nodeDiv.on('click', '.leftEdge, .rightEdge', function(event) {
      ...
      if (siblingsState.exist) {
        ...
        var nodeId = $that.parent()[0].id;
		var parentExist = getNodeState($node, 'parent').exist;
        var url = (parentExist) ?
        ...
        if (startLoading($that, $node, opts)) {
		  var tmpData;
		  if(parentExist) {
	        if($.isFunction(opts.ajaxURL.siblings)) {
			  tmpData = opts.ajaxURL.siblings(nodeData);
		    } else {
			  url = opts.ajaxURL.siblings + nodeId;
	        }
		  } else {
			if($.isFunction(opts.ajaxURL.families)) {
			  tmpData = opts.ajaxURL.families(nodeData);
		    } else {
			  url = opts.ajaxURL.families + nodeId;
	        }
	      }
		  if(tmpData.constructor === {}.constructor) {
			if (tmpData.siblings || tmpData.children) {
			  addSiblings($node, tmpData, opts);
			  endLoading($that, $node, opts);
			}
		  } else {
  ...
  function buildHierarchy ($appendTo, nodeData, level, opts, callback) {
    ...
    if (hasChildren) {
      ...
      var $nodeLayer;
      if (isVerticalLayer) {
        $nodeLayer = $('<ul>').addClass('nodes');
        if (isHidden) {
          $nodeLayer.addClass('hidden');
        }
  ...
  function buildChildNode ($appendTo, nodeData, opts, callback) {
    ...
    buildHierarchy($appendTo, { 'children': data }, nodeData.depth - 2, opts, callback);
  }
  // exposed method
  function addChildren($node, data, opts) {
    var opts = opts || $node.closest('.orgchart').data('options');
    var count = 0;
	var $appendTo = $node.closest('.orgchart').parent();
	data.depth = parseInt($node.attr('depth')) + 1;
    buildChildNode.call($appendTo, $node.closest('table'), data, opts, function() {
      if (++count === data.children.length) {
	    var isVerticalLayer = (opts.verticalDepth && (parseInt($node.attr('depth')) + 1) >= opts.verticalDepth) ? true : false;
		if(isVerticalLayer) {
  ...
  function buildSiblingNode($nodeChart, nodeData, opts, callback) {
    ...
    if ($nodeChart.parent().is('td')) {
      ...
      var nodeCount = 0;
      buildHierarchy($nodeChart.closest('.orgchart'), nodeData, parseInt(nodeData.level), opts, function() {
