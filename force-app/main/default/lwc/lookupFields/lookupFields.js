...
import getSobjectList from '@salesforce/apex/LookupFiledsController.getSobjectList';
...
export default class LookupField extends LightningElement {
    ...
    loadptions() {
        getSobjectList({
            childObjectApi: this.childObjectApi,
            refFieldName: this.refFieldName,
            fieldDisplayName: this.refFieldName,
            searchList: []
        })
            .then(result => {
                console.log('result of records ');
                console.log(result);
                this.options = result;
                
            }).catch(error=>{
                console.log('Function getSobjectList ERROR :'+JSON.stringify(error));
    ...
    handleSelection(event){
        ...
        this.dispatchEvent(new CustomEvent('change', { detail: {selectedOption: this.selectedOption, fieldLabel: this.fieldLabel } })); //Dispatch change event
    }
